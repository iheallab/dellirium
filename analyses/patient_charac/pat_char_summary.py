...
all_admissions.iloc[:, 5:22] = all_admissions.iloc[:, 5:22].fillna(0)

all_admissions['liver_disease'] = all_admissions[['mild_liver_dis', 'mod_sev_liver_dis']].apply(max, axis=1)
all_admissions['diabetes'] = all_admissions[['diabetes_wo_comp', 'diabetes_w_comp']].apply(max, axis=1)
...
for disease in comob:

    all_admissions.loc[all_admissions[disease] > 0, disease] = 1
...
for cohort in cohorts:
    ...
    for group in groups:
       ...
       # Numeric characteristics
        ...
        admissions_cohort_chars = pd.concat(
            [admissions_cohort_pat_count, admissions_cohort_num_skew, admissions_cohort_socio, admissions_cohort_bin],
            axis=0,
        )
    ...
    numerical_vars_static = ["age", "bmi", "cci", "icu_los"]
    ...
    categorical_vars = ["Female", "Black", "White", "Other"] + comob + ["coma", "died"]
    ...
    for var in categorical_vars:
        ...
        if count1 > 0 and count2 > 0:
            ...
            contingency_table = np.array([[count1, int(nobs1-count1)],
                                          [count2, int(nobs2-count2)]])
            ...
            if p_value < 0.005:
                ...
                cohort_df.loc[var, "p-value"] = f'{p_value:.2f}'
                
    print(cohort_df)
    cohort_df.to_csv(f"{ANALYSIS_DIR}/{cohort}_charac_summary.csv")
