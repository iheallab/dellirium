...
class SingleAttention(nn.Module):
    def __init__(self, attention_input_dim, attention_hidden_dim, demographic_dim, attention_type='add', time_aware=False, use_demographic=False):
    ...
    def forward(self, input, demo=None):
        ...
        time_decays = torch.tensor(range(23,-1,-1), dtype=torch.float32).unsqueeze(-1).unsqueeze(0).to(device)# 1*t*1
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        for i in range(DeCov_contexts.size(0)):
            Covs = cov(DeCov_contexts[i,:,:])
            DeCov_loss += 0.5 * (torch.norm(Covs, p = 'fro')**2 - torch.norm(torch.diag(Covs))**2 ) 
...
class ConCare(nn.Module):
    def __init__(self, input_dim, static_dim, hidden_dim, d_model,  MHD_num_head, d_ff, output_dim, keep_prob=0.5):
        ...
        self.LastStepAttentions = clones(SingleAttention(self.hidden_dim, 8, attention_type='new', demographic_dim=static_dim, time_aware=True, use_demographic=True),self.input_dim)
        ...
        self.demo_proj = nn.Linear(self.static_dim, self.hidden_dim)
