...
class ApricotM(nn.Module):
    def __init__(
        ...
        super(ApricotM, self).__init__()

        self.conv1d = nn.Sequential(
            nn.Conv1d(2, d_hidden, 1), nn.ReLU(), nn.Conv1d(d_hidden, d_model, 1)
        )

        self.embedding_variable = nn.Embedding(max_code + 1, d_model)
        ...
        self.mamba_block = MixerModel(
            d_model=d_model, n_layer=n_layer, vocab_size=max_code + 1
        )

        self.mlp = nn.Sequential(nn.Linear(d_model * 5, d_model), nn.Dropout(dropout))
    ...
    def forward(self, x, static):
        ...
        encoding = self.mamba_block(encoding)
