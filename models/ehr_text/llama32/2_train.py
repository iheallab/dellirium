...
# Load tokenizer
tokenizer = AutoTokenizer.from_pretrained(f"{LLM_DIR}/llama32_1b/", trust_remote_code=True)

if not tokenizer.pad_token:
    tokenizer.pad_token = tokenizer.eos_token
    tokenizer.pad_token_id = tokenizer.eos_token_id
    print(f"The tokenizer.pad_token set as a {tokenizer.pad_token}")
    print(f"The tokenizer.pad_token_id set as a {tokenizer.pad_token_id}")
...
if model.config.pad_token_id is None:
    ...
    print(f"The model.config.pad_token_id set as a {model.config.pad_token_id}")
...
# LoRA configuration
peft_params = LoraConfig(task_type="SEQ_CLS", inference_mode=False, r=16, lora_alpha=16, lora_dropout=0, use_rslora=True, target_modules=["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"])
