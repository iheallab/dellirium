...
tokenizer = AutoTokenizer.from_pretrained(f"{LLM_DIR}/clinicalbert/")
tokenizer.add_special_tokens({'pad_token': '[PAD]'})

model = AutoModelForSequenceClassification.from_pretrained(f"{LLM_DIR}/clinicalbert/", num_labels=2).to('cuda:0')
...
if FREEZE_BASE:
    ...
    for i in range(num_layers):
        ...
        for param in model.distilbert.transformer.layer[layer_id].parameters():
...
training_args = TrainingArguments(
    output_dir=f"{MODEL_DIR}/test_trainer/final2", 
