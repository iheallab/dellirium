...
def seq_to_text(group):
    ...
    for vital in vitals:
        ...
        vitals_report = vitals_report + f'{vital}: {vital_df["value"].min():.2f}-{vital_df["value"].max():.2f} [SEP] \n'

    vitals_report = "Vitals values: [SEP] \n" + vitals_report
    ...
    if len(labs) > 0:
        ...
        for lab in labs:
            ...
            labs_report = labs_report + f'{lab}: {lab_df["value"].min():.2f}-{lab_df["value"].max():.2f} [SEP] \n'
            
    else:
        
        labs_report = labs_report + "None. "
        
    labs_report = "Laboratory values: [SEP] \n" + labs_report
    ...
    if len(meds) > 0:
        ...
        for med in meds:
            ...
            med_report = med_report + f'{med}: {med_df["value"].sum():.2f} [SEP] \n'


    else:
        
        med_report = med_report + "None. [SEP] "
        
    med_report = "Medications: [SEP] \n" + med_report
    ...
    if len(scores) > 0:
        ...
        for score in scores:
            ...
            scores_report = scores_report + f'{score}: {score_df["value"].min():.2f}-{score_df["value"].max():.2f} [SEP] \n'
            
    else:
        
        scores_report = scores_report + "None. [SEP] "
        
    scores_report = "Score values: [SEP] \n" + scores_report
        
    report = vitals_report + "\n" + labs_report + "\n" + med_report + "\n" + scores_report
    
    # report = "This report contains a summary for the first 24h of an ICU admission:\n\n" + report
...
def static_to_text(row):
    ...
    comob_report = "Comorbidities at admission: [SEP] \n "
    ...
    for comob in comob_list:
        ...
        if row[comob] == "Yes":
            ...
            comob_report = comob_report + f"{comob}. [SEP] "
            
            count += 1
    
    if count == 0:
        
        comob_report = comob_report + "None. [SEP] "


    admit_report = f"Age: {int(row['Age'])} [SEP] \n" + f"Gender: {row['Gender']} [SEP] \n" + f"Race: {row['Race']} [SEP] \n"
...
# icustay_ids = seq["icustay_id"].unique()[:10]

# seq = seq[seq["icustay_id"].isin(icustay_ids)]
# static = static[static["icustay_id"].isin(icustay_ids)]
...
text.to_csv(f"{MODEL_DIR}/ehr_text_sec_ver2.csv", index=False)
